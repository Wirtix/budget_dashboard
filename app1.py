...
st.markdown('<h1 style="background-color:#092635; color:#FFFFFF;"> &nbsp;&nbsp Current month stats: </h1>', unsafe_allow_html=True)
st.markdown('---')
...
with m_column:
    ...
    st.subheader(f' :red[{now_month_spendings} ZŁ]')
with r_column:
    st.subheader(f'Left from {month_now[0].title()}:🤑')
    st.subheader(f' {now_month_balance} ZŁ')
st.markdown('---')

st.markdown('<h1 style="background-color:#092635; color:#FFFFFF;">&nbsp;&nbsp Other stats: </h1>', unsafe_allow_html=True)
...
fig_inc_by_cat = px.pie(
    df_melted, names='category', values='income_by_cat', title=f' Income by category in selected Month', color_discrete_sequence=custom_colors_1
)
fig_sp_by_cat = px.pie(
    df_melted_spend, names='category', values='spendings_by_cat', title=f' Spend by category in selected Month', color_discrete_sequence=custom_colors_2
)
